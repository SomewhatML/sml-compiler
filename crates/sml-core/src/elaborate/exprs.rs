...
impl Context {
    fn elab_if(&mut self, sp: Span, e1: Expr, e2: Expr, e3: Expr) -> Expr {
        let tru = Rule {
    ...
    fn elab_rule(&mut self, rule: &ast::Rule, bind: bool) -> Rule {
        let (pat, _) = self.elaborate_pat(&rule.pat, bind);
        let expr = self.elaborate_expr(&rule.expr);
        Rule { pat, expr }
    }

    pub fn elab_rules(&mut self, sp: Span, rules: &[ast::Rule]) -> (Vec<Rule>, Type) {
        self.with_scope(|f| {
            let rules = rules
                .into_iter()
    ...
    pub fn elaborate_expr(&mut self, expr: &ast::Expr) -> Expr {
        match &expr.data {
            ...
            ast::ExprKind::Case(scrutinee, rules) => {
                ...
                let arr = Type::arrow(Type::Var(self.fresh_tyvar()), Type::Var(self.fresh_tyvar()));
            ...
            ast::ExprKind::Const(c) => {
                ...
                Expr::new(ExprKind::Const(*c), ty, expr.span)
            }
            ...
            ast::ExprKind::If(e1, e2, e3) => {
                ...
                Expr::new(ExprKind::Case(Box::new(e1), vec![tru, fls]), ty, expr.span)
            }
            ast::ExprKind::Let(decls, body) => {
                ...
                Expr::new(ExprKind::Let(elab, Box::new(body)), ty, expr.span)
            }
            ast::ExprKind::List(exprs) => {
                ...
                Expr::new(ExprKind::List(exprs), ty, expr.span)
            }
            ...
            ast::ExprKind::Primitive(prim) => {
                ...
                Expr::new(ExprKind::Primitive(name), ty, expr.span)
            }
            ast::ExprKind::Raise(expr) => {
                ...
                Expr::new(ExprKind::Raise(Box::new(ex)), ty, expr.span)
            }
            ast::ExprKind::Record(rows) => {
                ...
                Expr::new(ExprKind::Record(rows), ty, expr.span)
            }
